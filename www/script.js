document.addEventListener('DOMContentLoaded', () => {
    ...
    convertBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            const progressPromise = setupEventSource(requestId).catch(error => {
                if (!downloadStarted) {
                    throw error;
                }
            });
            ...
            downloadStarted = true;
            const downloadPromise = startDownload(url, requestId);

            // Attendre que les deux promesses soient résolues
            await Promise.all([progressPromise, downloadPromise]);
